	...
	$query=mysqli_query($con,"SELECT DISTINCT * FROM spip_articles WHERE id_secteur=1  AND ((id_rubrique!=19 AND id_rubrique!=23) AND statut='publie') ORDER BY date DESC LIMIT 20");
		...
		{      
			while($r=mysqli_fetch_array($query)){
				...
				$video[]=array("postId"=>"$r[0]", "postTitle"=>"$r[2]","poster"=>"Tv1","post"=>"$post", "postTime"=>"$postTime", "visites"=>"$r[17]","postImage"=>"http://tv1.rw/IMG/$imgUrl","category"=>"$postCategory","views"=>$views,"description"=>"$r[5]","rubrique"=>$id_rubrique);
